...
This file records the reusable framework documents that control the Chicago 311 run.

If conversational memory conflicts with the current framework files, the framework files control.

| Stage | Framework | Repository path | Purpose |
|---|---|---|---|
...
At formal run start, record the commit/blob/retrieval version for each controlling framework. If a controlling framework changes materially during the project, document the change and determine whether any previously passed gate must be reopened.
